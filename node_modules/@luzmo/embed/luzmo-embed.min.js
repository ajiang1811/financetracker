...
var VERSION = "0.0.0-shared-luzmo";
var sanitizeUrl = (url) => {
...
var getFilters = (dashboardComponent, iframe) => {
  if (iframe?.contentWindow) {
    const requestId = uuidv4();
    iframe.contentWindow.postMessage(
      { action: "getFilters", _version: dashboardComponent.VERSION, requestId },
      "*"
    );
    return new Promise((resolve, reject) => {
      dashboardComponent.promises[requestId] = { resolve, reject };
...
var reloadDashboard = (dashboardComponent, iframe) => {
  if (iframe) {
    iframe.contentWindow.postMessage(
      { action: "reloadDashboard", _version: dashboardComponent.VERSION },
      "*"
    );
    return Promise.resolve();
  }
...
var LuzmoDashboard = class extends s3 {
  ...
  loadLibrary(appServer) {
    ...
    return __federationMethodGetRemote(appServer, "./DashboardApp").then((module2) => {
  ...
  setInputParameters(inputParameters) {
    ...
    if (inputParameters.appServer) {
      this.appServer = addTrailingSlash(inputParameters.appServer);
    }
  }
